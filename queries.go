...
const (
	...
	getProductIDList = " SELECT id " +
		" FROM product " +
		" WHERE parient_product_id = 0 and store_id = ? "
	...
	// insertProductSubSku = "INSERT into product_sub_sku(product_id, sub_sku, " +
	// 	" stock,size, color, weight, width, height, depth, cost, msrp, map, price,  " +
	// 	" sale_price, image1, image2, image3,image4) " +
	// 	" values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)"
	...
	getProductByCatAndManufacturer = "SELECT p.id, p.sku, p.gtin, p.name, p.short_description, p.description, " +
		...
		" inner join category c " +
		" on pc.category_id = c.id " +
		" WHERE p.parient_product_id = 0 and c.id = ? and p.manufacturer = ? and p.store_id = ? " +
	...
	getProductByCat = "SELECT p.id, p.sku, p.gtin, p.name, p.short_description, p.description, " +
		...
		" inner join category c " +
		" on pc.category_id = c.id " +
		" WHERE p.parient_product_id = 0  and c.id = ? ORDER BY p.name LIMIT ?, ? "
	...
	deleteSubProduct = "DELETE FROM product WHERE parient_product_id = ? "
	...
	insertOrder = "INSERT INTO orders (order_date, status, subtotal, shipping_handling, " +
		...
		" ffl_name, ffl_lic, ffl_exp_date, ffl_phone )" +
		" values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) "
	...
	updateOrder = "UPDATE orders SET updated = ?, status = ?, subtotal = ?, shipping_handling = ?, " +
		...
		" ffl_address = ?, ffl_name = ?, ffl_lic = ?, ffl_exp_date = ?, ffl_phone = ? " +
		" WHERE id = ? "
